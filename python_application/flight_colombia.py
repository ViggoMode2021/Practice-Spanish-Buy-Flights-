...
flight_colombia = Blueprint("flight_colombia", __name__)

@flight_colombia.route('/book_colombia_flight_page')
def book_colombia_flight_page():
    if 'loggedin' in session:
        ...
        return render_template("book_colombia_flight_page.html", overall_score=overall_score)

    return redirect(url_for('sign_up'))

@flight_colombia.route('/select_dates_for_colombia_trip', methods=['POST', 'GET'])
def select_dates_for_colombia_trip():
    if 'loggedin' in session:
        conn = psycopg2.connect(dbname=DB_NAME, user=DB_USER, password=DB_PASS, host=DB_HOST)
        cursor = conn.cursor(cursor_factory=psycopg2.extras.DictCursor)
        ...
        departure_years = request.form.get("departure_years_colombia")

        departure_months = request.form.get("departure_months_colombia")

        departure_days = request.form.get("departure_days_colombia")

        departure_date = departure_years + departure_months + departure_days

        departure_date_int = int(departure_date)

        departure_times = request.form.get("departure_times_colombia")

        return_years = request.form.get("return_years_colombia")

        return_months = request.form.get("return_months_colombia")

        return_days = request.form.get("return_days_colombia")

        return_date = return_years + return_months + return_days

        return_date_int = int(return_date)

        return_times = request.form.get("departure_times_colombia")
        ...
        return render_template("colombia_flight_pre_confirmation.html", overall_score=overall_score, departure_date=departure_date,
                               departure_times=departure_times, return_date=return_date, return_times=
                               return_times)

    return redirect(url_for('sign_up'))

@flight_colombia.route('/confirm_dates_for_colombia_trip', methods=['POST', 'GET'])
def confirm_dates_for_colombia_trip():
    if 'loggedin' in session:
        conn = psycopg2.connect(dbname=DB_NAME, user=DB_USER, password=DB_PASS, host=DB_HOST)
        cursor = conn.cursor(cursor_factory=psycopg2.extras.DictCursor)

        cursor.execute('SELECT departure_date FROM flights WHERE username = %s AND flight = %s;', (session['username'], 'Medellín, Colombia'))

        departure_date = cursor.fetchone()[0]

        print(departure_date)

        cursor.execute('SELECT departure_time FROM flights WHERE username = %s AND flight = %s;', (session['username'], 'Medellín, Colombia'))

        departure_time = cursor.fetchone()[0]

        cursor.execute('SELECT return_date FROM flights WHERE username = %s AND flight = %s;', (session['username'], 'Medellín, Colombia'))

        return_date = cursor.fetchone()[0]

        cursor.execute('SELECT return_time FROM flights WHERE username = %s AND flight = %s;', (session['username'], 'Medellín, Colombia'))
        ...
        if not flight_exists:
            ...
            cursor.execute(
                ...
                ('True', date_today, current_time, session['username'], "Medellín, Colombia"))

            new_overall_score = overall_score - 350
            ...
            return redirect(url_for('flight_colombia.select_dates_for_colombia_trip'))

        return render_template("colombia_trip_confirmation_page.html", departure_date=departure_date, departure_time=departure_time,
